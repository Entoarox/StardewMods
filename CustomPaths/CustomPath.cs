...
{
    using Framework;
    class CustomPath : TerrainFeature, ICustomItem, IDeserializationHandler
    {
        private static Dictionary<byte, int> DrawGuide = new Dictionary<byte, int>()
        {
            [0] = 0,
            [2] = 1,
            [8] = 2,
        ...
        {
            this.Connection = GetTileIndex(position, ignore);
        ...
        internal void UpdateNeighbors(Vector2 position, bool ignore=true)
        {
            for (int y = -1; y < 2; y++)
                for (int x = -1; x < 2; x++)
                {
                    if (x == 0 && y == 0)
                        continue;
                    var vector = new Vector2(position.X + x, position.Y + y);
                    if (Game1.currentLocation.terrainFeatures.TryGetValue(vector, out var obj) && obj is CustomPath path && path.Id.Equals(this.Id))
                        if (ignore)
        ...
        {
            return Game1.currentLocation.terrainFeatures.TryGetValue(new Vector2(x, y), out var obj) && obj is CustomPath path && path.Id.Equals(this.Id) && !(ignore is Vector2 skip && x == skip.X && y == skip.Y);
        }
        private int GetTileIndex(Vector2 position, Vector2? ignore=null)
        {
            ...
            for(int y=-1;y<2;y++)
                for(int x=-1;x<2;x++)
                {
                    ...
                    if (mult == 0)
                        ...
                        mult *= 2;
                    if (!HasPath(position.X + x, position.Y + y, ignore))
                        continue;
                    if (Math.Abs(x)==1 && Math.Abs(y)==1)
                    {
                        bool first, second;
                        if (x == -1)
                            first = HasPath(position.X + x + 1, position.Y + y, ignore);
                        else
                            first = HasPath(position.X + x - 1, position.Y + y, ignore);
                        if (y == -1)
                            second = HasPath(position.X + x, position.Y + y + 1, ignore);
                        else
                            second = HasPath(position.X + x, position.Y + y - 1, ignore);
                        if (first && second)
                            tiles += mult;
            ...
            int index= DrawGuide.ContainsKey(tiles) ? DrawGuide[tiles] : 0;

...
namespace Entoarox.Framework.Core.Injection
{
    ...
    class Crop_newDay
    {
        ...
        {
            if (!EntoaroxFrameworkMod.Config.GreenhouseEverywhere &&
                (!(environment is IAugmentedLocation) || !(environment as IAugmentedLocation).IsGreenhouse))
                return true;
            if (state == 1)
            {
            ...
            if ((__instance.fullyGrown.Value && __instance.dayOfCurrentPhase.Value > 0) ||
                ...
                __instance.rowInSpriteSheet.Value != 23) return false;
